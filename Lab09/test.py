...
def follow_line(frame, drone, direction, prev_angle):
    ...
    if contours:
        ...
        if M["m00"] > 0:
            ...
            time.sleep(0.5)
        # if abs(cx - frame_center) < dthreshold and abs(cy - frame_center) < dthreshold:  # 對準中心
        #     drone.send_rc_control(direction[0], direction[1], direction[2], direction[3])
        #     print(f"Follow line: ({direction[0]}, {direction[1]}, {direction[2]}, {direction[3]})")
        #     time.sleep(0.5)
        # else:
        #     drone.send_rc_control(int((cx - frame_center)/10), 0, int((cy - frame_center)/10), 0)
        #     print(f"Adjust center: ({direction[0]}, {direction[1]}, {direction[2]}, {direction[3]})")
        #     time.sleep(0.5)
...
while True:
    ...
    start_row = h // 4  # 上方裁剪 1/4
    end_row = start_row + h // 2  # 中間的 1/2
    start_col = w // 4  # 左右裁剪 1/4
    end_col = start_col + w // 2  # 中間的 1/2
    ...
    # blur_gray = cv2.GaussianBlur(threshold_frame, (kernel_size, kernel_size), 0)
    ...
    # threshold_value = 127  # 閾值
    # max_value = 255  # 超過閾值的像素設為此值
    # _, threshold_frame = cv2.threshold(erode_frame, threshold_value, max_value, cv2.THRESH_BINARY)
    
    # h, w = threshold_frame.shape
    # start_row = h // 4  # 上方裁剪 1/4
    # end_row = start_row + h // 2  # 中間的 1/2
    # start_col = w // 4  # 左右裁剪 1/4
    # end_col = start_col + w // 2  # 中間的 1/2

    # # line_frame = threshold_frame
    # line_frame = threshold_frame[start_row:end_row, start_col:end_col]
